...
- (UITableView*)tableView {
    ...
    return nil;
}//要求在继承时一定要重写tableView这个方法，因为BaseListViewController上没有tableView
...
- (void)loadWithLoadingView {
    ...
    _loadingStatus = LoadingStatusLoadingWithLoadingView;
    [self loadData];
}

- (void)loadWithRefreshView {
    _loadingStatus = LoadingStatusLoadingWithRefreshView;
    [self loadData];
}

- (void)loadWithToastActivity {
    [self.view makeToastActivity:CSToastPositionCenter];
    _loadingStatus = LoadingStatusLoadingWithToastActivity;
    [self loadData];
}
...
- (void)removeLoadingViews {
    if (_loadingStatus == LoadingStatusLoadingWithLoadingView) {
        if (_loadingView) {
            [_loadingView removeFromSuperview];
        }
    } else if (_loadingStatus == LoadingStatusLoadingWithRefreshView) {
    ...
    } else if (_loadingStatus == LoadingStatusLoadingWithToastActivity) {
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset {
    ...
    if (_loadingStatus == LoadingStatusIdle && self.tableView.contentOffset.y < -kRefreshViewHeight - scrollView.contentInset.top) {
        ...
        targetContentOffset->y = -kRefreshViewHeight - self.tableView.contentInset.top;
        [_refreshView beginRefreshing];
        [_refreshView setVisibleHeight:kRefreshViewHeight];
        
        [self loadWithRefreshView];
    }
}
...
- (void)showErrorViewWithMessage:(NSString*)message {
    ...
    [promotingMsgLabel.topAnchor constraintEqualToAnchor:errorMsgLabel.bottomAnchor constant:10].active = YES;
    [promotingMsgLabel.centerXAnchor constraintEqualToAnchor:_errorView.centerXAnchor].active = YES;
