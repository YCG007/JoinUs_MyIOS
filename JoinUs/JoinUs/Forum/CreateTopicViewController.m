...
@property (weak, nonatomic) IBOutlet UITextField *topicTitleTextField;
@property (weak, nonatomic) IBOutlet UITextView *topicContentTextView;
@property (nonatomic, strong) UICollectionView *collectionView;
@property (weak, nonatomic) IBOutlet UIView *photoView;
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *topicAddPhotoViewBottomConstraint;
...
- (void)viewDidLoad {
    ...
    //插入背景图片
    self.view.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@" all_background.png"]];
    
    //To make the border look very close to a UITextField
    [self.topicContentTextView.layer setBorderColor:[[[UIColor grayColor] colorWithAlphaComponent:0.5] CGColor]];
    [self.topicContentTextView.layer setBorderWidth:1.0];
    
    //The rounded corner part, where you specify your view's corner radius:
    self.topicContentTextView.layer.cornerRadius = 5;
    self.topicContentTextView.clipsToBounds = YES;
    
    self.topicAddPhotoViewBottomConstraint.constant = -self.photoView.frame.size.height;
}
...
- (IBAction)pickPhotoButtonPressed:(id)sender {
    self.topicAddPhotoViewBottomConstraint.constant = 0;
    
    countLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, 32, 21)];
    NSString* countString = [[NSString alloc]initWithFormat:@"%lu / 9",(unsigned long)_selectedAssets.count];
    [countLabel setText:countString];
...
- (void)submitTopic {
    ...
    if (_uploadedImageCount <= _selectedAssets.count) {
        ...
        topicAdd.title = self.topicTitleTextField.text;
        
        PostAdd* postAdd = [[PostAdd alloc]init];
        postAdd.content = self.topicContentTextView.text;
        postAdd.imageIds = [_selectedPhotos copy];
        topicAdd.firstPost = postAdd;
        
        NSString *url = [NSString stringWithFormat:@"topic"];
        [[NetworkManager sharedManager] putDataWithUrl:url data:[topicAdd toJSONData] completionHandler:^(long statusCode, NSData *data, NSString *errorMessage) {
            [self.view hideToastActivity];
            if (statusCode == 200) {
