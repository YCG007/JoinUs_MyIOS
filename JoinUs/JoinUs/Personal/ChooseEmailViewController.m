//
//  ChooseEmailViewController.m
//  JoinUs
//
//  Created by 杨春贵 on 16/5/6.
//  Copyright © 2016年 North Gate Code. All rights reserved.
//

#import "ChooseEmailViewController.h"
#import "Utils.h"
#import "NetworkManager.h"

@interface ChooseEmailViewController ()
...
@implementation ChooseEmailViewController
...
- (void)viewDidLoad {
    ...
    self.oldEmailLabel.text = myProfile.email;
...
- (IBAction)resendVerifyCodeButtonPressed:(id)sender {
    ...
    NSString* url = [NSString stringWithFormat:@"myProfile/emailVerifyCode"];
...
- (IBAction)registerSubmitButtonPressed:(id)sender {
    ...
    emailVerifyCode.email = self.oldEmailLabel.text;
    emailVerifyCode.verifyCode = self.verifyCodeTextField.text;
    
    [[NetworkManager sharedManager] postDataWithUrl:@"myProfile/verifyEmail" data:[emailVerifyCode toJSONData] completionHandler:^(long statusCode, NSData *data, NSString *errorMessage) {
        [self.view hideToastActivity];
        if (statusCode == 200) {
            NSError* error;
            ...
            [self.view makeToast:msg.message duration:1.0f position:CSToastPositionCenter title:nil image:nil style:nil completion:^(BOOL didTap) {
                [self performSegueWithIdentifier:@"GoToNewEmail" sender:self];
            }];
            if (error == nil) {
